...
function twitter_update() {
  ...
  if ($status) {
    ...
    $post_data = array('source' => '3927352233', 'status' => $status);
...
function twitter_comment($query) {
  ...
  if (is_numeric($id)) {
    ...
    $post_data = array('source' => '3927352233', 'comment' => $comment, 'id' => $id);
    $b = twitter_process($request, $post_data);
  }
  twitter_refresh($_POST['from'] ? $_POST['from'] : '');
}
...
function twitter_comments_page() {
  $request = 'http://twitter.com/statuses/comments_timeline.json?page='.intval($_GET['page']);
  $tl = twitter_process($request);
  $tl = twitter_standard_timeline($tl, 'replies');
  $content = theme('status_form');
  $content .= theme('timeline', $tl);
...
function theme_action_icons($status) {
  ...
  if (!$status->is_direct) {
   ...
   $actions[] = theme('action_icon', "comment/{$status->id}", 'images/retweet.png', 'CMT');
    if (user_is_current_user($from)) {
