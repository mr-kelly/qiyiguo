 ...
 ?>
 			 <?php
 			 	//加入、关注按钮
 			 ?>
			 <a class="medium blue awesome join_group_btn" href="<?=site_url("group/ajax_join_group/". $group["id"]);?>"><?= $group['privacy'] == 'public' ? '关注' : '加入'; ?></a>
 ...
 <?php
		 ...
		 echo '已加入';
 ?>
  	<a href="<?=site_url('group/ajax_join_group/' . $group['id'] .'/exit');?>" class="awesome join_group_btn">退出该群</a>
 	
 <?php
		 if ($ci->group_model->is_group_admin($group['id'], get_current_user_profile('id'))):
 ?>
			,管理员 <!--<a href="<?=site_url('group/setting/'.$group['id']);?>">果群设置</a>-->
